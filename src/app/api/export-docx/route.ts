...
import { exportEstimateToDocx, exportContractToDocx } from "@/lib/export-docx-pdf";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === "contract") {
      ...
      // Use provided totals or calculated
      ...
      const blob = await exportContractToDocx(data, calculation, finalContractData);

      return new NextResponse(blob as any, {
        status: 200,
        headers: {
          "Content-Disposition": `attachment; filename="dogovor_${finalContractData.contractNumber}.docx"`,
      ...
      const blob = await exportEstimateToDocx(data, calculation);
